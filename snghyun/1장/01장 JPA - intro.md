...
객체가 단순하면 데이터베이스에 모든 속성을 꺼내 저장하면 되지만, 상속 혹은 참조하고 있다면 저장하기가 쉽지 않다.
...
객체지향 모델링을 사용하면 객체를 테이블에 저장하거나 조회하기 쉽지 않다.
...
### 왜 JPA를 사용해야 하는가?
1. 생산성
	1. 자바 컬렉션에 저장하듯이 JPA에게 저장할 객체를 된다.
...
3. 패러다임의 불일치 해결
	1. JPA는 상속 연관관계, 객체 그래프 탐색, 비교하기와 같은 패러다임의 불일치 문제를 해결해준다.
4. 성능
